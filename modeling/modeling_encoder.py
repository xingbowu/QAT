...
class LSTMTextEncoder(nn.Module):
    ...
    def __init__(self, vocab_size=1, emb_size=300, hidden_size=300, output_size=300, num_layers=2, bidirectional=True,
                 ...
                 pool_function='max', concat_choice=False, output_hidden_states=False):
        super().__init__()
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, model_name, output_token_states=False, from_checkpoint=None, **kwargs):
        ...
        if 'concat_choice' in kwargs :
            ...
            self.concat_choice = False
        if 'output_attentions' in kwargs :
            self.output_attn = kwargs['output_attentions'] if kwargs['output_attentions'] is not None else False
        else :
        ...
        if self.model_type in ('lstm',):
            ...
            print('---'*10)
            self.module = AutoModel.from_pretrained(model_name, output_attentions=self.output_attn,
            ...
            self.max_position_embeddings = self.module.config.max_position_embeddings
            if self.concat_choice :
                 # first 5 : choice embedding / last : the rest
                self.choice_embed = nn.Embedding(6, self.module.config.hidden_size).to(self.module.device)

    def reform_input(self, inputs):
        input_ids, attention_mask, token_type_ids, output_mask = inputs
        num_choices = 5 # TODO should generalize
        reformed_input_ids, reformed_attention_mask, reformed_token_type_ids, reformed_output_mask = [], [], [], []
        position_ids, choice_ids = [], []
        for i in range(0, input_ids.shape[0], num_choices):
            start = attention_mask[i].sum().item()
            pos_ids = torch.arange(input_ids.shape[1], device=input_ids.device)
            ch_ids = torch.zeros(input_ids.shape[1], device=input_ids.device)
            ch_ids[((input_ids[i] == 2).cumsum(0) == 2) & (((input_ids[i] == 2).cumsum(0) == 2).cumsum(0) > 1)] = 1
            for j in range(1,num_choices):
                choice_id = input_ids[i+j][(input_ids[i+j] == 2).cumsum(0) == 2][1:]
                input_ids[i][start:start+choice_id.shape[0]] = choice_id
                input_ids[i][start+choice_id.shape[0]] = 2
                attention_mask[i][start:start+choice_id.shape[0]+1] = 1
                output_mask[i][start:start+choice_id.shape[0]] = False
                pos_ids[start:start+choice_id.shape[0]] = self.max_position_embeddings - j
                ch_ids[start:start+choice_id.shape[0]] = j+1
                start += choice_id.shape[0]+1
            reformed_input_ids.append(input_ids[i])
            reformed_attention_mask.append(attention_mask[i])
            reformed_token_type_ids.append(token_type_ids[i])
            reformed_output_mask.append(output_mask[i])
            pos_ids[output_mask[i]] = 0
            position_ids.append(pos_ids)
            choice_ids.append(ch_ids)
        return torch.stack(reformed_input_ids), \
               torch.stack(reformed_attention_mask), \
               torch.stack(reformed_token_type_ids), \
               torch.stack(position_ids), \
               torch.stack(choice_ids), \
               torch.stack(reformed_output_mask)
    ...
    def forward(self, *inputs, layer_id=-1):
        ...
        return sent_vecs, hidden_states, (attention_mask, output_mask)
